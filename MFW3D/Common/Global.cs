                ...
                switch (result)
                {
                    case DialogResult.Yes:
                        ...
                        if (zipFilePath.EndsWith("xml"))
                        {
                            ...
                            string targetLocation = MFW3D.Global.Settings.DirectoryPath + Path.DirectorySeparatorChar + "Config" + Path.DirectorySeparatorChar + worldWindow.CurrentWorld.Name + Path.DirectorySeparatorChar + source.Name;
                        ...
                        try
                        {
                            ...
                            fz.ExtractZip(zipFilePath, MFW3D.Global.Settings.DirectoryPath, "");
                        ...
                        try
                        {
                            string ManifestFile = Path.Combine(MFW3D.Global.Settings.DirectoryPath, "manifest.txt");
                            if (File.Exists(ManifestFile))
                            {
                                ...
                                while ((line = fs.ReadLine()) != null)
                                {
                                    ...
                                    if (line.Length > 0)
                                    {
                                        ...
                                        FileInfo fi = new FileInfo(MFW3D.Global.Settings.DirectoryPath + Path.DirectorySeparatorChar + line);
                                ...
                                fs.Close();
                                File.Delete(Path.Combine(MFW3D.Global.Settings.DirectoryPath, "manifest.txt"));
                            }
                            else
                            {
                ...
                if (String.Compare(paramValuePair[0], "wmsimage", true, CultureInfo.InvariantCulture) == 0)
                {
                    ...
                    try
                    {
                        ...
                        string texturePath = string.Format(CultureInfo.InvariantCulture,
                            @"{0}\Data\DownloadedWMSImages\{1}", MFW3D.Global.Settings.DirectoryPath, System.DateTime.Now.ToFileTimeUtc());
