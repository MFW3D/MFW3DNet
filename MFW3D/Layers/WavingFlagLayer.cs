...
namespace MFW3D.Renderable
{
    public class WavingFlagLayer : RenderableObject
    {
        ...
        static VertexDeclaration m_vertexDeclaration = null;
        ...
        private void renderHighlight(DrawArgs drawArgs)
        {
            ...
            drawArgs.device.Transform.World *= Matrix.Scaling(World.Settings.VerticalExaggeration * ScaleX,
                World.Settings.VerticalExaggeration * ScaleY, World.Settings.VerticalExaggeration * ScaleZ);
        ...
        public override void Render(DrawArgs drawArgs)
        {
            ...
            if (m_polygonFeature == null || !drawArgs.WorldCamera.ViewFrustum.Intersects(
                    m_polygonFeature.BoundingBox))
                return;
            try
            {
                double offset = 0;
        ...
        private void RenderFlag(DrawArgs drawArgs, double offset)
        {
            ...
            Vector3 pos = MathEngine.SphericalToCartesian(m_latitude, m_longitude, 
                World.EquatorialRadius + World.Settings.VerticalExaggeration * ScaleZ + offset);
            ...
            drawArgs.device.Transform.World = Matrix.Scaling(World.Settings.VerticalExaggeration * ScaleX * 0.01f, 
                World.Settings.VerticalExaggeration * ScaleY * 0.01f, -World.Settings.VerticalExaggeration * 2 * ScaleZ);
            drawArgs.device.Transform.World *= Matrix.RotationY((float)-MathEngine.DegreesToRadians(90));
            drawArgs.device.Transform.World *= Matrix.RotationY((float)-MathEngine.DegreesToRadians(m_latitude));
            drawArgs.device.Transform.World *= Matrix.RotationZ((float)MathEngine.DegreesToRadians(m_longitude));
            drawArgs.device.Transform.World *= Matrix.Translation(surfacePos - rc);
