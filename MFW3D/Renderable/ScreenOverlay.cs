...
namespace MFW3D.Renderable
{
	...
	public class ScreenOverlay : MFW3D.Renderable.RenderableObject
	{
		...
		public override void Initialize(DrawArgs drawArgs)
		{
			if(overlay == null)
			{
				overlay = new MFW3D.Widgets.Form();
				overlay.Text = name;
                ParentControl_Resize(null, null);
                overlay.OnVisibleChanged += new MFW3D.Widgets.VisibleChangedHandler(overlay_OnVisibleChanged);

                overlay.AutoHideHeader = !m_ShowHeader;
				overlay.HideBorder = m_HideBorder;
				overlay.BorderColor = m_BorderColor;

				pBox = new MFW3D.Widgets.PictureBox();
