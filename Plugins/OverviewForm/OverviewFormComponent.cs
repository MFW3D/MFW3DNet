...
namespace MFW3D.CMPlugins.OverviewForm
{
	...
	public class OverviewFormComponent : System.Windows.Forms.Control
	{
		...
		private void getRenderButtons(MFW3D.Renderable.RenderableObject ro, System.Collections.ArrayList renderList)
		{
			if(ro.MetaData != null)
			{
		...
		private void RenderImageLayer(MFW3D.Renderable.ImageLayer imageLayer)
		{
		...
		private void RenderQuadTileSet(MFW3D.Renderable.QuadTileSet quadTileSet)
		{
			try
			{
		...
		private void RenderQuadTile(MFW3D.Renderable.QuadTile quadTile)
		{
...
//				RenderImageTileCacheEntry(tileEntry);
		...
		private void RenderImages(MFW3D.Renderable.RenderableObjectList rol)
		{
			if(rol.IsOn)
			{
				for(int i = 0; i < rol.ChildObjects.Count; i++)
				{
					MFW3D.Renderable.RenderableObject curObject = (MFW3D.Renderable.RenderableObject)rol.ChildObjects[i];
					if(curObject is MFW3D.Renderable.RenderableObjectList)
					{
						RenderImages(curObject as MFW3D.Renderable.RenderableObjectList);
					}
					if(curObject.IsOn)
					{
						if(curObject is MFW3D.Renderable.ImageLayer)
						{
						
							MFW3D.Renderable.ImageLayer curImageLayer = (MFW3D.Renderable.ImageLayer)curObject;
							RenderImageLayer(curImageLayer);
						
						}
						else if(curObject is MFW3D.Renderable.QuadTileSet)
						{
							MFW3D.Renderable.QuadTileSet curQuadTileSet = (MFW3D.Renderable.QuadTileSet)curObject;
							RenderQuadTileSet(curQuadTileSet); 
						}
						else if(curObject is MFW3D.Renderable.TerrainPath)
						{
							try
							{
								MFW3D.Renderable.TerrainPath curTerrainPath = (MFW3D.Renderable.TerrainPath)curObject;
						...
						else if(curObject is MFW3D.Renderable.TiledPlacenameSet)
						{
							try
							{
								MFW3D.Renderable.TiledPlacenameSet curTiledPlacenameSet = (MFW3D.Renderable.TiledPlacenameSet)curObject;
						...
						else if(curObject is MFW3D.Renderable.Icon)
						{
							try
							{
								MFW3D.Renderable.Icon curIcon = (MFW3D.Renderable.Icon)curObject;
								RenderIcon(curIcon);
							}
							catch(Exception ex)
							{
						...
						else if(curObject is MFW3D.Renderable.DownloadableImageFromIconSet)
						{
							try
							{
								MFW3D.Renderable.DownloadableImageFromIconSet iconSet = (MFW3D.Renderable.DownloadableImageFromIconSet)curObject;
		...
		private void RenderDownloadableImageFromIconSet(MFW3D.Renderable.DownloadableImageFromIconSet iconSet)
		{
			if(iconSet.isInitialized)
			{
				MFW3D.Renderable.DownloadableIcon[] icons = iconSet.DownloadableIcons;
				
				foreach(MFW3D.Renderable.DownloadableIcon icon in icons)
				{
					...
					if(iconResource == null)
					{
						...
						iconResource.Object = new MFW3D.Renderable.IconTexture(m_Device3d, icon.IconFilePath);

						m_ResourceCache.Add(icon.IconFilePath, iconResource);
					}

					MFW3D.Renderable.IconTexture iconTexture = (MFW3D.Renderable.IconTexture)iconResource.Object;
		...
		private void RenderIcon(MFW3D.Renderable.Icon icon)
		{
			if(icon.isInitialized && icon.MaximumDisplayDistance > 100000 && icon.MaximumDisplayDistance != 0)
			{
				...
				{
					iconResource = new ResourceCacheEntry();
					iconResource.Object = new MFW3D.Renderable.IconTexture(m_Device3d, icon.TextureFileName);
		...
		private void RenderTiledPlacenameSet(MFW3D.Renderable.TiledPlacenameSet tiledPlacenameSet)
		{
		...
		private void RenderTerrainPath(MFW3D.Renderable.TerrainPath terrainPath)
		{
			//if(terrainPath != null)
			{
