...
namespace Stars3D.Plugin
{
	...
	public class Stars3DLayer : RenderableObject
	{
		...
		/// Constructor
		/// </summary>
		public Stars3DLayer(string LayerName, string pluginPath, MFW3D.WorldWindow worldWindow) : base(LayerName)
		{
		...
		public void ReadSettings()
		{
			...
			if(line != "")
			{
                ...
                if(settingsList.Length >= 4) IsOn = (settingsList[3] == "False") ? false : true;
                //save this to check whether we need to change the ini settings on Dispose
                m_initialIsOnFlag = IsOn;
		...
		{
			string line = version + ";" + catalogFileName + ";" + showFlares.ToString() + ";" + IsOn;
			try
			{
		...
		{
            //this saves whether you had Starfield checked in LayerManager or not when unloading(?) 
            //the Plugin or Exiting WorldWind.
            if (IsOn != m_initialIsOnFlag)
            {
                SaveSettings();
            }
