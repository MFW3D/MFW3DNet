...
namespace NLT.Plugins
{
	...
	/// </summary>
	public class ShapeFileLoader : MFW3D.PluginEngine.Plugin
	{
		...
		public void loadShapeFileWithAlreadyExistingXML(string shapeConfigFilePath, bool checkForUpdate)
		{
			...
			if(layersetIter.Count > 0)
			{
				while(layersetIter.MoveNext())
				{
					...
					MFW3D.Renderable.RenderableObjectList newLayerSetList
					...
					if(shapeIter.Count > 0)
					{
						while(shapeIter.MoveNext())
						{
							...
							MFW3D.ShapeFillStyle shapeFillStyle = getShapeFillStyleFromString(getInnerTextFromFirstChild(shapeIter.Current.Select("PolygonFillStyle")));
							...
							MFW3D.ShapeFileLayer shapeFileLayer = new ShapeFileLayer(
								name,
		...
		static MFW3D.ShapeFillStyle getShapeFillStyleFromString(string shapeFillStyleString)
		{
